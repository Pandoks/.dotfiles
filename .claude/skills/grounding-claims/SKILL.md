...
description: ALWAYS invoke this skill BEFORE writing any answer that contains a factual claim, recommendation, technical advice, or piece of casual advice — code or non-code, including fitness, lifting, biomechanics, medical, nutrition, historical, legal, financial, software, library, API, or version Q&A. Triggers include any question starting with "why / how does / what causes / is it true / should I / compare / does X support / how long / what's the difference / is it safe / is it worth / side effects / dose / interactions"; any "someone told me X" framing; any answer that names a specific number, version, dose, threshold, or named entity the user might act on; any code recommendation that hasn't been run. Symptoms include answering "does X support Y / which version / how does Z behave" from memory, giving advice resting on an unchecked fact, citing a library option or signature from memory, claiming/denying a config key exists without seeing the schema, hedging ("where did you see that?") instead of looking it up. DO NOT skip because the question feels casual, the answer feels well-known, the topic is "just biomechanics / just lifting / just nutrition / just opinion", or you "already grounded something this turn" — each distinct claim needs its own check. Skip ONLY for: explicitly-framed speculation ("I think X, let me check"), facts the user already stated as a premise, or true long-stable built-ins (basic POSIX, git plumbing, ECMAScript core).
---
